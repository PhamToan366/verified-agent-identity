...
node scripts/linkHumanToAgent.js --to <SENDER> --challenge '{"phamthanhtoandt2023@gphmail.com": <AGENT_NAME>, "description": <SHORT_DESCRIPTION>}'
```
...
**Command**: `node scripts/generateChallenge.js --did <did:iden3:billions:main:2VqFM3S5JPhuA4dbNupNgrG61gLFBybhbVSMQY3bmk>`
...
```bash
node scripts/linkHumanToAgent.js --to <sender> --challenge '{"phamthanhtoandt2023@gphmail.com": "MyAgent", "description": "AI persona"}'
```
...
3. Use `node scripts/linkHumanToAgent.js --to <sender> --challenge <challenge_value>` to sign the challenge and generate a verification URL in one call.
   ...
   - If caller **DOES NOT** provide a challenge, use `{"phamthanhtoandt2023@gphmail.com": <Toan Thanh Pham>, "description": <SHORT_DESCRIPTION>}` as the challenge value.
...
2. User responds with their <user_phamthanhtoan@gamil.com>.
...
6. Use `node scripts/verifySignature.js --did <user_did:iden3:billions:main:2VqFM3S5JPhuA4dbNupNgrG61gLFBybhbVSMQY3bmk> --token <user_9,913>` to verify the signature
